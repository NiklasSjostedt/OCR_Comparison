...
# Constants
BASE_DIR = r'C:\Users\nikla\Desktop\Examensarbete\OCR_Comparison\SciTSR\2images'
...
def create_dataframe_and_save(ocr_texts, json_texts, processing_times, sorted_text=False):
    ...
    DistanceEditDict = {}
    file_name = 'EasyOCR_Refactored.xlsx' if sorted_text else 'EasyOCR_Refactored_No_SORT.xlsx'
...
def process_files(image_dir, json_dir):
    ...
    for filename in os.listdir(image_dir):
        if filename.endswith('.png'):
            ...
            ocr_text = ' '.join(t[1] for t in result).split()
            join_ocr_text = ''.join(ocr_text)
            re_ocr_text = re.sub(PATTERN, '', join_ocr_text)
            sorted_ocr_text = ''.join(sorted(ocr_text))
            sorted_ocr_text_join = re.sub(PATTERN, '', sorted_ocr_text)
            ocr_texts.append(re_ocr_text)
            sorted_ocr_texts.append(''.join(sorted(sorted_ocr_text_join)))
    ...
    for json_filename in os.listdir(json_dir):
        ...
        join_json_text = ''.join(json_text)
        sorted_json_text = ''.join(sorted(join_json_text))
        json_texts.append(''.join(json_text))
