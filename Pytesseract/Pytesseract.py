...
# Constants
pytesseract.pytesseract.tesseract_cmd = r'C:\Users\nikla\Desktop\Examensarbete\AbeNickDocs\Tesseract\tesseract.exe'
...
def create_dataframe_and_save(ocr_texts, json_texts, processing_times, sorted_text=False):
    ...
    DistanceEditDict = {}
    file_name = 'Pytesseract_Refactored.xlsx' if sorted_text else 'Pytesseract_Refactored_No_SORT.xlsx'
...
def process_files(image_dir, json_dir):
    ...
    for filename in os.listdir(image_dir):
        if filename.endswith('.png'):
            ...
            ocr_text = result
            sorted_ocr_text = re.sub(PATTERN, '', ocr_text)
            sorted_ocr_text_join = ''.join(sorted(sorted_ocr_text))
            ocr_texts.append(re.sub(PATTERN, '', ocr_text))
            sorted_ocr_texts.append(sorted_ocr_text_join)
